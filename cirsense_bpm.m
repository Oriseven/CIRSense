...
results = repmat(struct( ...
    ...
    "dylign_time", NaN, ...
    "cirsense_total_time", NaN, ...
    "diversense_time", NaN, ...
    "diversense_total_time", NaN, ...
    "farsense_time", NaN), 1, numel(matchingFiles));
...
for result_idx = 1:numel(matchingFiles)
    ...
    tstart = tic;
    data_rmm = csi_data_processed - mean(csi_data_processed, 2);
    smooth_data = movmean(data_rmm, window_length, 2);
    [respiration_rate_div, diversense_time] = diversense(smooth_data, t_slice);
    diversense_total_time = toc(tstart);
    disp("Estimated bpm by DiverSense: " + num2str(respiration_rate_div) + ...
        "  GT: " + num2str(respiration_rate_gt) + ...
        " Difference = " + num2str(abs(respiration_rate_gt - respiration_rate_div)) + ...
        "  using " + num2str(diversense_time) + " and " + num2str(diversense_total_time));

    tstart = tic;
    respiration_rate_far = farsense(csi_data_processed, t_slice, window_length, params);
    farsense_time = toc(tstart);
    disp("Estimated bpm by FarSense: " + num2str(respiration_rate_far) + ...
        "  GT: " + num2str(respiration_rate_gt) + ...
        " Difference = " + num2str(abs(respiration_rate_gt - respiration_rate_far)) + ...
        " using " + num2str(farsense_time));

    results(result_idx) = struct( ...
        "file_name", string(FileName), ...
        "estimated_dis", estimated_dis, ...
        "respiration_rate_gt", respiration_rate_gt, ...
        "cirsense_bpm", respiration_rate_cir, ...
        "diversense_bpm", respiration_rate_div, ...
        "farsense_bpm", respiration_rate_far, ...
        "preprocess_time_per_frame", preprocess_time_per_frame, ...
        "dylign_time", dylign_time, ...
        "cirsense_total_time", cirsense_total_time, ...
        "diversense_time", diversense_time, ...
        "diversense_total_time", diversense_total_time, ...
        "farsense_time", farsense_time);
...
params.nrx = 2;                     % Number of receiver antennas
...
params.n_target = 1;                % Number of targets to detect
params.sel_target = 1;              % Index of target to focus on
...
matchingFiles = unique(file_names(mask), "stable");
end
...
function respiration_rate = farsense(data, t_slice, window_length, params)
[nsc, ntime] = size(data);
H_all_sc = zeros(size(data));
bnr_sc = zeros(1, nsc);

for sc = 1:nsc
    smooth_data = sgolayfilt(data(sc,:),3,window_length);
    [H_all_sc(sc, :), bnr_sc(sc)] = angle_projection_core( ...
        smooth_data, t_slice, ntime, params.respiration_range_hz, params.n_fft);
end

max_lag = floor(ntime / 3) * 2;
autocorr_results = zeros(nsc, max_lag + 1);
for sc = 1:nsc
    autocorr_results(sc, :) = calculate_autocorrelation(H_all_sc(sc, :), max_lag);
end

[max_bnr, ~] = max(bnr_sc);
selected_subcarriers = find(bnr_sc > 0.7 * max_bnr);

% Combine autocorrelation results weighted by BNR
weighted_autocorr = zeros(1, max_lag + 1);
for i = selected_subcarriers
    weighted_autocorr = weighted_autocorr + bnr_sc(i) * autocorr_results(i, :);
end

[peaks, peak_locs] = findpeaks(weighted_autocorr, "MinPeakProminence", 0.1);
...
function [respiration_rate, tend] = diversense(smooth_data, t_slice)

tstart = tic;
% Calculate optimal phase rotation for each subcarrier
clean_phase = rotation(smooth_data);
% Combine signals using optimized phase rotations
combined_data = exp(1j * 2 * pi .* clean_phase.') * smooth_data ./ numel(clean_phase);
tend = toc(tstart);

% Apply PCA to extract principal component of the signal
shift_data = [real(combined_data); imag(combined_data)];
[~, score, ~] = pca(shift_data.', "NumComponents", 1);
projected_sig = score.';

respiration_rate = estimate_respiration_rate(projected_sig, t_slice);
end
...
function clean_phase = rotation(smooth_data)
% Calculate optimal phase rotation for each subcarrier relative to a reference
[nSc, ~] = size(smooth_data);
ref_f = 1;
clean_phase = zeros(nSc,1);
for s = 1:nSc
    objective = @(params) sum( abs( smooth_data(ref_f,1:10:end) - smooth_data(s,1:10:end).*exp(1j*2*pi*params(1)) ).^2 );
